...
describe('graph store', () => {
	...
	describe('updateNodePosition', () => {
		...
		it('should handle API errors gracefully', async () => {
			...
			// Check that saving state is cleared
			const clearedState = get(graphStore);
			expect(clearedState.savingNodes.has('node1')).toBe(false);
		...
		it('should handle multiple concurrent position updates', async () => {
			// Add nodes to the store first
			const state = get(graphStore);
			['node1', 'node2', 'node3'].forEach((id, index) => {
				state.nodes.set(id, {
					id,
	...
	describe('loadViewport', () => {
		...
		it('should update existing nodes with new data', async () => {
			...
			// Verify metadata exists before checking its properties
		...
		it('should handle API errors when loading viewport', async () => {
			const { toast } = await import('./toast');
			...
			await expect(graphStore.loadViewport(viewport, 1)).rejects.toThrow();
			expect(toast.error).toHaveBeenCalledWith('Failed to load graph data. Please refresh the page.');
		});

		it('should handle invalid JSON response from viewport API', async () => {
			const { toast } = await import('./toast');
		...
		it('should load edges along with nodes', async () => {
			const mockData = {
				nodes: [
					{ id: 'node1', title: 'Node 1', position: { x: 0, y: 0 }, level: 1 },
					{ id: 'node2', title: 'Node 2', position: { x: 100, y: 100 }, level: 1 }
				],
				edges: [
					{ id: 'edge1', source: 'node1', target: 'node2', weight: 1 },
					{ id: 'edge2', source: 'node2', target: 'node1', weight: 2 }
				]
			};
			...
			const viewport = { minX: 0, maxX: 200, minY: 0, maxY: 200 };
			await graphStore.loadViewport(viewport, 1);
			...
			expect(state.nodes.has('node2')).toBe(true);

			// Verify edges are loaded
			expect(state.edges.size).toBe(2);
			expect(state.edges.has('edge1')).toBe(true);
			expect(state.edges.has('edge2')).toBe(true);

			// Verify edge properties
			const edge1 = state.edges.get('edge1');
...
describe('fetchWithRetry utility', () => {
	...
	it('should use exponential backoff delay', async () => {
		...
		// fetchWithRetry returns the last failing response, not throws
		const response = await fetchWithRetry('/test-endpoint', {}, 3, 50);
		expect(response.status).toBe(500);
	...
	it('should pass through request options', async () => {
		...
		// Capture full request details for comprehensive assertions
		server.use(
		...
		expect(capturedBody).toEqual({ test: 'data' });
	});
});

describe('edge cases', () => {
	it('should handle concurrent node removal during position update', async () => {
		// Set up initial node
		const state = get(graphStore);
		state.nodes.set('node1', {
			id: 'node1',
		...
		server.use(
			http.put('/api/v1/nodes/node1/position', () => {
				// Simulate node being removed while API call is in flight
				const currentState = get(graphStore);
				currentState.nodes.delete('node1');
				return new HttpResponse(null, { status: 200 });
			})
		);
		...
		// Verify the node was removed and position update didn't crash
		const finalState = get(graphStore);
		expect(finalState.nodes.has('node1')).toBe(false);
		expect(finalState.savingNodes.has('node1')).toBe(false);
	});
});
