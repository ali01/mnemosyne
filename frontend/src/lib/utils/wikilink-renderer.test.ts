...
describe('WikiLink renderer', () => {
	...
	afterEach(() => {
		// Reset marked configuration to prevent test pollution
		marked.setOptions(marked.getDefaults());
	});
	...
	describe('wikilinkExtension', () => {
		...
		it('should handle nested brackets with current regex limitations', async () => {
			...
			// The wikilink will include "Note [with brackets" and the remaining "]]" will be plain text
			expect(html).toContain('<a href="/notes/Note%20%5Bwith%20brackets" class="wikilink" data-note="Note [with brackets">Note [with brackets</a>');
	...
	describe('XSS prevention', () => {
		it('should escape HTML in note names and aliases', async () => {
			const markdown = 'Malicious: [[<script>alert("XSS")</script>|<img src=x onerror=alert("XSS")>]].';
			const html = await marked(markdown);

			// Verify script tags are escaped and won't execute
			expect(html).not.toContain('<script>');
			expect(html).not.toContain('<img src=x');
			// The content should be escaped in the href attribute
			expect(html).toContain('%3Cscript%3E');
			// The display text should be escaped
			expect(html).toContain('&lt;img');
		});

		it('should escape various XSS vectors in wikilinks', async () => {
			const xssVectors = [
				'[[<iframe src="javascript:alert(1)"></iframe>]]',
				'[[javascript:alert("XSS")]]',
				'[[<svg onload=alert(1)>]]',
				'[[<img src=x onerror=alert(1)>|Innocent Text]]',
				'[[Normal Note|<script>alert(1)</script>]]'
			];

			for (const vector of xssVectors) {
				const html = await marked(vector);

				// Verify no executable HTML tags are present
				expect(html).not.toContain('<script');
				expect(html).not.toContain('<iframe');
				expect(html).not.toContain('<svg');
				expect(html).not.toContain('<img ');

				// Verify dangerous content is escaped or encoded
				if (vector.includes('<script>')) {
					expect(html).toContain('&lt;script&gt;');
				}
				if (vector.includes('javascript:')) {
					// Should be URL encoded in href
					expect(html).toContain('javascript%3A');
				}
			}
		});

		it('should properly escape data-note attribute', async () => {
			const markdown = '[[Note with " quotes and \'apostrophes\']]';
			const html = await marked(markdown);

			// Verify quotes are properly escaped in attributes
			expect(html).toContain('data-note="Note with &quot; quotes and &#39;apostrophes&#39;"');
			// Also verify the href is properly encoded
			expect(html).toContain('href="/notes/Note%20with%20%22%20quotes%20and%20\'apostrophes\'"');
		});

		it('should handle encoded XSS attempts', async () => {
			const markdown = '[[%3Cscript%3Ealert(1)%3C/script%3E]]';
			const html = await marked(markdown);

			// Even encoded attempts should be handled safely
			expect(html).not.toContain('<script>');
